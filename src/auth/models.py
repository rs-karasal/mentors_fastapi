...
class AccountType(Base):
    ...
    permissions: Mapped[str] = mapped_column(String(length=100), nullable=True)
...
class User(SQLAlchemyBaseUserTable[int], Base, AbstractModel):
    ...
    account_type_id: Mapped[int] = mapped_column(Integer, ForeignKey("account_types.id"), nullable=False, default=1)
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    is_superuser: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    is_verified: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)

    account_types = relationship("AccountType", back_populates="user")
    # messages = relationship("Message", back_populates="user")
    # chat_member = relationship("ChatMember", back_populates="user")
